...
export const Route = createFileRoute("/_authenticated/admin/payouts")({
  beforeLoad: async ({ context }) => {
    ...
    if (!ok) throw redirect({ to: "/forbidden" });
  },
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Review, approve and settle host payout requests for the LUMORO X PARK marketplace.",
      },
      ...
      <header className="border-b border-border/60 bg-background/60 backdrop-blur">
        <div className="mx-auto flex max-w-5xl items-center gap-2 px-5 py-4">
          <Button asChild variant="ghost" size="sm">
            <Link to="/admin"><ArrowLeft className="mr-1 h-4 w-4" />Admin</Link>
          </Button>
            ...
            {payouts.map((p) => (
              <li key={p.id} className="rounded-2xl border border-border bg-card p-5 shadow-card">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="font-semibold">{p.host_name ?? "Host"} · {money(p.amount)}</div>
