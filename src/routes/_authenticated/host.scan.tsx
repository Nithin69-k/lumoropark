      ...
      <header className="border-b border-border/60 bg-background/60 backdrop-blur">
        <div className="mx-auto flex max-w-2xl items-center gap-3 px-5 py-4">
          <Button asChild variant="ghost" size="sm">
            <Link to="/host"><ArrowLeft className="mr-1 h-4 w-4" />Host dashboard</Link>
          </Button>
      ...
      <main className="mx-auto max-w-md px-5 py-10">
        <div className="rounded-3xl border border-border bg-card p-6 shadow-card">
          ...
          <p className="mt-1 text-center text-sm text-muted-foreground">
            Ask the renter to open their booking and read out the 16-character check-in code (or scan it later — camera scan coming soon).
          </p>
          ...
          <div className="mt-6 space-y-3">
            ...
            <Button className="w-full" size="lg" onClick={handleCheckIn} disabled={busy || !code.trim()}>
          ...
          {ok && (
            <div className="mt-5 flex items-start gap-3 rounded-xl border border-success/30 bg-success/5 p-3 text-sm">
              ...
              <div>
                ...
                <div className="text-muted-foreground">{ok.renter_name} · {ok.space_title}</div>
              </div>
            </div>
          )}
        </div>
