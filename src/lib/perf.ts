...
function push(event: PerfEvent) {
  ...
  if (limit !== undefined && event.value > limit) {
    console.warn(`[perf] slow ${event.name}: ${Math.round(event.value)}${event.unit === "ms" ? "ms" : ""}`, event.detail ?? "");
  }
}
...
export function markPerf(name: string, detail?: Record<string, unknown>, value = 0, unit: PerfEvent["unit"] = "ms") {
...
export function startPerfTimer(name: string, detail?: Record<string, unknown>) {
  ...
  return (extra?: Record<string, unknown>) => {
    ...
    push({ name, value: ended - started, unit: "ms", at: Date.now(), detail: { ...detail, ...extra } });
  };
}
...
export function initPerfMonitoring() {
  ...
  const observe = (type: string, cb: (entries: PerformanceEntryList) => void, extra?: PerformanceObserverInit) => {
    try {
