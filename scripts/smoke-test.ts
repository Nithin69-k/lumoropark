 ...
 *   BASE_URL=https://lumoropark.lovable.app bun run smoke
 *
...
async function main() {
  ...
  await step("home page renders hero + primary CTA", async () => {
    ...
    assert(await page.getByRole("link", { name: /find|browse|spot/i }).first().count(), "no browse CTA");
  });

  await step("browse page loads map frame and listings", async () => {
    await open("/browse");
    await page.waitForSelector(".leaflet-container, [role='status'], [role='alert']", { timeout: 20_000 });
  ...
  await step("auth page exposes sign in, sign up and forgot password", async () => {
    ...
    assert(await page.getByLabel(/password/i).first().isVisible(), "no password field");
  ...
  await step("performance metrics are collected", async () => {
    ...
    const events = await page.evaluate(() => (window as unknown as { __lumoroPerf?: () => unknown[] }).__lumoroPerf?.() ?? []);
