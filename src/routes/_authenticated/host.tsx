      ...
      <main className="mx-auto max-w-5xl px-5 py-8">
        {isLoading ? (
          ...
          <div className="rounded-3xl border border-dashed border-border bg-card/50 p-12 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Add your first parking space to start earning.</p>
            <Button asChild className="mt-6">
              <Link to="/host/new">
                <Plus className="mr-1 h-4 w-4" /> List a space
              </Link>
          ...
          <div className="grid gap-5 md:grid-cols-2">
            {spaces.map((s) => (
              <SpaceCard key={s.id} space={s} onToggle={(active) => toggle.mutate({ id: s.id, active })} />
            ))}
          </div>
        )}
      </main>
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-card">
      <div className="relative h-40 bg-muted">
        {space.photos[0] ? (
          <SpacePhoto path={space.photos[0]} alt={space.title} className="h-full w-full object-cover" />
        ) : (
          <div className="flex h-full items-center justify-center text-muted-foreground">
            <MapPin className="h-8 w-8" />
          </div>
        )}
        <Badge className="absolute right-3 top-3" variant={space.is_active ? "default" : "secondary"}>
        ...
        <div className="space-y-2">
          ...
          <div className="max-h-56 overflow-y-auto rounded-lg border border-border">
            {!slots || slots.length === 0 ? (
              <p className="p-4 text-center text-sm text-muted-foreground">No slots yet — space is unavailable.</p>
            ) : (
              <ul className="divide-y divide-border">
                {slots.map((slot) => (
                  <li key={slot.id} className="flex items-center justify-between px-3 py-2 text-sm">
                    <span>
                      {format(new Date(slot.start_time), "MMM d, HH:mm")} → {format(new Date(slot.end_time), "MMM d, HH:mm")}
                    </span>
